...
**Варианты на потом:** (а) взять целиком и перестроить сетку под переменные высоты; (б) пропустить и зафиксировать вечную дельту по всем `portal.swift`; (в) гибрид — забрать `Kit/module/portal.swift` + модульные порталы, а `reinit()` оставить свой, адаптировав под `p.height`. Склонялись к (в), решение отложено до завершения текущей работы по двухколоночным попапам.
