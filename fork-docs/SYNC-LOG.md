...
## 2026-08-21 — post-v3.0.11 катч-ап: взяты все 7 коммитов (билд остаётся 834)
...
`MARKETING_VERSION` не трогали — апстрим стоит на **3.0.11**, новых тегов нет. `CFBundleVersion` остался **834**: деплоя в этой сессии не было, номер не «потрачен» (то же правило, что 09.08).

Ad-hoc Debug-сборка по рецепту из `SIGNING.md` — **BUILD SUCCEEDED** под Xcode 16.0, новых trailing-comma костылей не потребовалось.

> ⚠️ Release-сборка и деплой в `/Applications` **не делались** — в дереве по-прежнему незавершённый WIP по двухколоночным попапам (снапшот `3b0dd907`), установленная копия остаётся билдом **834** от 09.08.
